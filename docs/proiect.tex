...
\textit{Notă}: Proiectul suportă și deployment pe \textbf{Oracle Autonomous Transaction Processing (ATP) Free Tier}
(2 instanțe cloud, protocol TCPS/mTLS, DB Links prin \texttt{DBMS\_CLOUD\_ADMIN}),
disponibil prin scriptul \texttt{scripts/setup-all.sh}.
...
și este conectat doar prin DB Links standard pe rețeaua bridge \texttt{globalbank-net}.
...
\textbf{Tip replicare}: sincronă, tranzacțională, \textbf{single-master}
(Central → noduri locale; modificările se propagă automat în aceeași tranzacție
prin triggerul \texttt{TRG\_REPLICATE\_TIPURI\_CONT})
...
\subsection{Schema globală (GLOBAL\_USER — container Central:1521)}
...
\textbf{Locale (DDL, same container)}:
\begin{lstlisting}[caption={FK-uri locale}]
CONTURI_B.ID_Tip     -> TIPURI_CONT.ID_Tip    (FK local pe containerul Bucharest)
CONTURI_B.ID_Client  -> CLIENTI_ID.ID_Client  (FK local pe containerul Bucharest)
PLATI_RATE.ID_Credit -> CREDITE.ID_Credit     (FK local pe containerul Central)
\end{lstlisting}
...
-- TRG_BEF_INS_CREDITE (pe Central):
...
\begin{itemize}
  \item JOIN distribuit pe 3 vederi globale (accesează simultan containerul Central și cele 2 noduri locale)
  \item Filtru pe \texttt{CLIENTI\_PROFIL.Scor\_Credit} (stocată pe Central)
  \item Agregare cross-fragment (\texttt{SUM}, \texttt{COUNT} pe date de pe ambele noduri locale via DB Links)
\end{itemize}
...
\lstinputlisting[caption={sql/docker/global/00-setup-global-user.sql},style=sqlstyle]{../sql/docker/global/00-setup-global-user.sql}
\screenshot{N2-bd/01-users/01-global-user-created.png}{GLOBAL\_USER creat cu succes pe containerul Central}
...
\screenshot{N2-bd/01-users/02-bucharest-user-created.png}{BUCHAREST\_USER creat cu succes pe containerul Bucharest}
...
\screenshot{N2-bd/01-users/03-cluj-user-created.png}{CLUJ\_USER creat cu succes pe containerul Cluj}

\subsection{DB Links (GLOBAL\_USER pe Central)}
\lstinputlisting[caption={sql/docker/global/01-db-links.sql},style=sqlstyle]{../sql/docker/global/01-db-links.sql}
\screenshot{N2-bd/01-users/04-db-links-verified.png}{BUCHAREST\_LINK și CLUJ\_LINK verificate cu SELECT USER FROM DUAL@...}
...
Aplicația Flask se conectează exclusiv la \texttt{GLOBAL\_USER} pe containerul Central (port 1521)
și accesează datele de pe celelalte două containere prin DB Links, ca și când nu ar exista distribuire.
